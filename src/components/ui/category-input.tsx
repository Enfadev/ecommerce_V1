...
const defaultCategories = [
  ...
  "Technology", "Fashion", "Furniture", "Kitchen", "Music",
  "Gaming", "Jewelry", "Art & Crafts", "Pet Supplies", "Office"
];
...
export function CategoryInput({
  ...
  const filteredCategories = allCategoryNames.filter(categoryName =>
  ...
  const handleInputChange = (newValue: string) => {
    ...
    const exactMatch = allCategoryNames.find(cat => 
  ...
  const isNewCategory = (categoryName: string) => {
    return !allCategoryNames.some(cat => 
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className="w-full p-0" align="start">
          <Command>
            <CommandInput
              ref={inputRef}
              placeholder="Search categories or type new one..."
              value={inputValue}
            ...
            <CommandList>
              <CommandEmpty>
                <div className="flex flex-col items-center gap-2 p-4">
                  ...
                  {inputValue.trim() && (
                    <Button
                      size="sm"
                      onClick={() => createNewCategory(inputValue.trim())}
                      className="gap-2"
                      disabled={loading}
                    >
                  ...
                  return (
                    ...
                    >
                      ...
                      <Check
                        className={cn(
                          "ml-auto h-4 w-4",
                          value === categoryName ? "opacity-100" : "opacity-0"
                        )}
                      />
                    </CommandItem>
                  );
              ...
              {inputValue.trim() && isNewCategory(inputValue) && (
                <CommandGroup heading="Create New">
                  <CommandItem
                    onSelect={() => createNewCategory(inputValue.trim())}
