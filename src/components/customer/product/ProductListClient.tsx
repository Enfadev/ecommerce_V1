...
export function ProductListClient({ initialProducts, searchQuery = "", categoryFilter = "All", priceFilter = "all" }: ProductListClientProps) {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    let filtered = initialProducts.filter((p) => p.name.toLowerCase().includes(localSearch.toLowerCase()));
    ...
    if (priceFilter !== "all") {
      filtered = filtered.filter((p) => {
        if (priceFilter.startsWith("lt")) {
          const max = Number(priceFilter.replace("lt", ""));
          return p.price < max;
        }
        if (priceFilter.startsWith("gt")) {
          const min = Number(priceFilter.replace("gt", ""));
          return p.price > min;
        }
    ...
    return filtered;
  }, [initialProducts, localSearch, categoryFilter, priceFilter, sortBy]);
