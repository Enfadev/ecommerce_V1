...
export const productSchema = z.object({
  ...
  price: z.number().positive("Price must be positive").max(999999.99, "Price must not exceed 999,999.99"),

  stock: z.number().int("Stock must be a whole number").min(0, "Stock cannot be negative").max(999999, "Stock must not exceed 999,999"),
  ...
  categoryId: z.number().int("Category ID must be a whole number").positive("Category ID must be positive").optional(),
